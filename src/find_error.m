function errblock = find_error(contents)
rule_num = 4; % number of rule
error_row = zeros(rule_num,1);

% Error 1: The calculated reflux ratio is less than 0 or the number of plates is less than 0.
% Possibly because the light and heavy key components are too easy to separate.
...
% Error 3: The design specification does not converge, possibly because the initial value is near
% the optimal solution resulting in a step size smaller than the convergence accuracy
error_string(3,:) = ["ERROR WHILE EXECUTING CONVERGENCE BLOCK: ""$OLVER", ...
    "* WARNING WHILE EXECUTING CONVERGENCE BLOCK: ""$OLVER"];
locate = contains(contents,error_string(3,:));
error_line = find(locate == 1);
if ~isempty(error_line)
    for i = 1:length(error_line)
        error_row(3,i) = error_line(i);
        error_contents{3,i} = char(contents(error_line(i)-3)); % error -> -2，warning -> -3
    end
end
    ...
    for j = 1:c
        if error_row(i,j) ~= 0
            ...
            switch i
                ...
                case 3
                    % Preceding spaces are not captured when grabbing strings
