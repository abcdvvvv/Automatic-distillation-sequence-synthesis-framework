...
if number == 1
    ...
    for k = 1:number - 1
        ...
        if name_count>=5 %做case2苯胺这里要改成0 不合并塔
            ...
            if ~isempty(l)
                if columnio{l,6}==1 %第一次添加selector
                    ...
                    strname=[num2str(l),'-2']; %例如,S3-2
                    stream.Elements.Add(['S',strname]);
                    block.FindNode(['M',num2str(l),'\Ports\F(IN)']).Elements.Add(['S',strname]);
                    %S - (2)连接到上一个DUPL(当前塔的出料流股)
                    switch p
                        case 1
                    ...
                    repeat=0;
                    strname=[num2str(l),'-',num2str(columnio{l,6}+1)]; %例如,S3-3
                    switch p
                        case 1
                            for j=2:length(dupl(:,1)) %可能出现连入既有塔后后续分离任务也全是既有塔的情况,此时不重复生成流股
                                if strcmp(dupl_D_last,dupl{j,1}) %例如B22连第一个混合塔，后续也重复时该轮的dupl会变，如B24，此时B24不存在，此时说明不用生成流股
                                    stream.Elements.Add(['S',strname]);
                                    block.FindNode(['M',num2str(l),'\Ports\F(IN)']).Elements.Add(['S',strname]);
                                    %S -N连接到上一个DUPL
                                    block.FindNode([dupl_D_last,'\Ports\P(OUT)']).Elements.Add(['S',strname]);
        ...
        if azeotrope==0
            ...
            %当萃取剂为外加物质时，将其添加到material中，只对下一层以后生效
