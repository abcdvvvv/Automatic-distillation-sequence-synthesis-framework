...
while a
                ...
                if j == length(errblock(i,:)) || (j ~= length(errblock(i,:)) && errblock(i,j+1) == 'T')
                    ...
                    switch i
                        ...
                        case 3 % handle ERROR3
                            % if DS.FindNode('T1D\Output\FINAL_VAL\1').value-DS.FindNode('T1D\Output\INIT_VAL\1').value<1e-5
    ...
    debug = debug + 1;
    if debug > 5
        disp(['Error occurs after 5 rounds of adjustments, please check manually. ' ...
            'Fix it and continue with run completion status.'])
        pause();
        a = 0;
    end
end
