function [CAPEX,F_min] = calc_capex_by_regress(optim_col,F,y,streamf,DHVL)
...
E_T = 0.55; % Plate column efficiency
if streamf(1) < 300
    D_lower_bound = 0.51;
else
    D_lower_bound = 0.36;
...
for i = 1:column_num
    ...
    rho_L = stream.FindNode([columnio{i,4},'\Output\RHOMX_MASS\MIXED']).value; % ρ_liquid
    MW = stream.FindNode([columnio{i,4},'\Output\MW']).value; % molecular weight
    rho_V = MW/22.4; % ρ_vapor
    u_max = C*sqrt((rho_L - rho_V)/rho_V); % max empty column gas velocity
    u = floor(u_max*10)/10;

    stages = double(optim_col(i).stage-2); % NT
    ...
    while F_min(i) < 0.5*streamf(i) && ~success
        ...
        for j = 1:length(D)
            % To make the fit better use continuous plate spacing
            interval = 0.0543*D(j) + 0.3283;
            L = actual_stages*interval;
            cost = calc_capex_column(T_operation,P_operation,D(j),stages,L,column_material_type,plate_material_type);
    ...
    if success == 0
        ...
        cost = calc_capex_column(T_operation,P_operation,D,stages,L,column_material_type,plate_material_type);
