...
newtext{1}=['"PS-1" ? ; "',unit,'_MOLE" ; \ P1 ID = DHVLMX UNITS = ( "kJ/kmol" ) \ \ P2 SYSPRES = YES'];
newtext{2}=['"PS-2" ? ; "',unit,'_MOLE" ; \ P1 ID = TCMX UNITS = ( C ) \ \ P2 SYSPRES = YES'];
newtext{3}=['"PS-3" ? ; "',unit,'_MOLE" ; \ P1 ID = PBUB UNITS = ( bar ) \ \ P1 ID = PDEW UNITS = ( bar ) \ \ P2 TEMP = ( 45. <22> <4> ) SYSPRES = YES SYSTEMP = NO'];
for i=1:3
